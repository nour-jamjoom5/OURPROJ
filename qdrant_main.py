# qa_chatbot.py
# docker run -p 6333:6333 -p 6334:6334 -v "C:/Users/mokar/KAUST Camp/project/OURPROJ/qdrant_storage:/qdrant/storage:z" qdrant/qdrant
...
# Your HybridRetriever class is good, but the implementation needs to be
# inside the `_get_relevant_documents` method.
...
persist_path   = "./qdrant_metadata_store"
...
#     return {"relevant_ids": {"$in": control_ids}}
...
def control_id_filter(user_query: str):
    ...
    normalized_ids = [normalize_control_id(cid) for cid in control_ids]

    return Filter(
        should=[
            FieldCondition(
                key="relevant_ids",
                match=MatchAny(any=normalized_ids)
            )
        ]
    )
...
def hybrid_control_aware_retriever(user_query, retriever, embedding_model, top_k=10):
    ...
    # return retriever._get_relevant_documents(user_query)
